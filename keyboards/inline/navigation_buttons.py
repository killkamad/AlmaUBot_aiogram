...
async def inline_keyboard_contacts_center():
    ...
    markup.add(
        *[InlineKeyboardButton(text=item['name_contact_center'],
                               callback_data=nav_center_callback.new(name=item["name_contact_center"]))
...
def inline_keyboard_pps():
    ...
    callback_button3 = InlineKeyboardButton(text="Школа предпринимательства и инноваций",
...
def inline_keyboard_old_building_back(building_callback, floor_callback):
    markup = InlineKeyboardMarkup(row_width=1)
    callback_button = InlineKeyboardButton(text="⬅ Назад",
