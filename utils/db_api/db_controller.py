    ...
    try:
        sql_select = """
              SELECT
               ...
               WHERE usr.idt = $1
               """
        record: Record = await pool.fetch(sql_select, user_id)
        return record
    except(Exception, ErrorInAssignmentError) as error:
        logging.info(error)
...
async def add_certificate_data(id_Telegram, id_request, id_certif, name_certif):
    ...
    try:
        async with pool.acquire() as connection:
            ...
            record: Record = await pool.fetchrow(sql_ex, int(id_Telegram), int(id_request), str(id_certif), str(name_certif))
...
async def delete_certificate_button(name_certif):
    ...
    try:
        async with pool.acquire() as connection:
            ...
            record: Record = await pool.fetchrow(sql_ex, str(name_certif))
